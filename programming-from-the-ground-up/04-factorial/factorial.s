...
.globl _start
.globl facotrial  # this is unnneededunless we want to share this function
                  # among other programs

_start:
    pushl $4      # This factorial take one argument - the number for which
                  # we want to find the factorial. So, it gets pushed

    call factorial    # run the factorial function
    addl $4, %esp     # Scubs the parameter that was pushed onto the stack
...
factorial:
    ...
    cmpl $1, %eax        # If the number is 1, that is our base case, and we 
                         # simply return (1 is already in %eax as the return
                         # value) 
    je end_factorial
    ...
    imull %ebx, %eax     # multiply that by the result of the last call to
                         # factorial (in %eax). The answer is stored in %eax,
                         # which is good since that is whree return values go
end_factorial:
    movl %ebp, %esp     # standard function return stuff - we have to restore
    popl %ebp           # %ebp and %esp to where they were before the function
                        # started
